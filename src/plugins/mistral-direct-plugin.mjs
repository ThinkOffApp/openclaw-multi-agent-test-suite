...
async function callMistral(messages, modelId, apiKey, timeoutMs) {
...
export function createMistralPlugin(options) {
  const {
    modelId,
    apiKey,
    timeoutMs = 60000
  ...
  if (!modelId) throw new Error('createMistralPlugin requires modelId');
  if (!apiKey) throw new Error('createMistralPlugin requires apiKey');
  ...
  return {
    ...
    async createSession(input) {
      ...
      async function chat(userMessage) {
        ...
        const text = await callMistral(conversationHistory, modelId, apiKey, timeoutMs);
