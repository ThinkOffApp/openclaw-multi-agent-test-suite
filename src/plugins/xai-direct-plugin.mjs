...
async function callXAI(messages, modelId, apiKey, timeoutMs) {
...
export function createXAIPlugin(options) {
  const {
    modelId,
    apiKey,
    timeoutMs = 60000
  ...
  if (!modelId) throw new Error('createXAIPlugin requires modelId');
  if (!apiKey) throw new Error('createXAIPlugin requires apiKey');
  ...
  return {
    ...
    async createSession(input) {
      ...
      async function chat(userMessage) {
        ...
        const text = await callXAI(conversationHistory, modelId, apiKey, timeoutMs);
